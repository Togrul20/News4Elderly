...
export const getStaticPaths = async () => {
  const res = await fetch(`https://newsdata.io/api/1/news?apikey=${process.env.NEWSDATA_KEY}&country=az,us,tr,ru,in&category=technology`);
  const result = await res.json();

  const paths = result.results.map((el) => {
    return {
      params: { technologytitle: convertToSlug(el.title) },
    };
  });
...
export const getStaticProps = async (context) => {
  const title = context.params.technologytitle;
